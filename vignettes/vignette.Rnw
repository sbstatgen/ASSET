...
cor option.
<<h.traits>>=
res <- h.traits(SNPs, studies, beta, sigma, ncase=case, ncntl=control, meta=TRUE)
@
...
<<h.types>>=
ret <- h.types(dat=data, response.var="TYPE", snp.vars=snp.vars, adj.vars=adj.vars, types.lab=types.lab, cntl.lab="CONTROL",
               logit=TRUE)
@
